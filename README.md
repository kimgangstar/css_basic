...
* 수열(nth) 선택자 
    ...
    * `형제선택자:nth-child(n)` (띄어쓰기금지!)
    * `형제선택자:nth-child(n)` (n번째 형제)
    * `형제선택자:nth-chiid(odd)` : 홀수
    * `형제선택자:nth-chiid(even)` : 짝수
...
* `margin` : 바깥쪽(형제 사이) 여백(피그마 기준**간격**) 
    ...
    * `margin:10px 20px 30px ` 상하 좌우 / 하
    * `margin:10px 20px  40px` 상/우/좌/하 (시계방향)
    **위 4개 속성 방향은 패딩도 동일하다.**
...
* `table {width:300px;}` 열이 3개일 경우 > 100씩 자동 분배
    ...
    * 전체 너비 300중 첫번째 열만 50을 가지고 나머지값은 나머지 열들에게 자동 분배
    * (열 안 내용크기에 따라 달라짐)
...
* `<button type="button" 이벤트="자바스크립트명령어작성">`
    ...
    * `windiow.location.href='실행주소'`
        *(위)`a href="실행주소"`와 동일한 js 명령어
    * `button:hover {}` : 버튼에 마우스를 올렸을 시 디자인 변경  

    # 웹 글꼴 `<link>`, `@font-face`
    ## `link` 사용법과 특징
    * `head`태그 안 `reset.css` 연결보다 위에 작성
    * `@font-face`에 비해 사용이 간편함
    * 작성한 html에서만 사용할 수있다는 단점이 있음.
    ...
    * `src:url(글꼴주소);`<!-- 필수 -->
    * `font-weigth:'글꼴굵기(200~700글꼴에 따라 다름)'` <!-- 선택 -->
...
### 메인축 정렬 속성
* `justify-contents:`
    * `flex_start` : 메인축이 수직이면 위쪽 , 수평이면 왼쪽
    * `flex-end` : 메인축이 수직이면 아래쪽 , 수평이면 오른쪽
...
## transition
* `:hover` 선택자로 인해 기존속성에서 다른속성으로 변경되는 css에 에니메이션을 주고 싶을 때
...
* `선택자 {animation:a 1s ease 0.5s alternate 3;}`
    * 특징 선택자에 0.5초 기다리고 1초동안 a 애니메이션을 ease 가속도로 정방향 -> 역방향 순서로 3번 재생하기 (무한 재생은 `infinite`)
